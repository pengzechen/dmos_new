...
static uint64_t load_elf_file(process_t *pro, const char *elf_file_addr, pte_t * page_dir) {
    ...
    for (int i = 0; i < elf_hdr->e_phnum; i++, e_phoff += elf_hdr->e_phentsize) {
        ...
        if (elf_phdr.p_type != 1 || elf_phdr.p_vaddr ) {
            continue;
        }
        ...
        // 设置堆的起始和结束地址
        pro->heap_start = elf_phdr.p_vaddr + elf_phdr.p_memsz;
...
void process_init(process_t * pro, void * elf_addr) {
    ...
    tcb_t * main_thread = create_task((void (*task_func)())pro->entry, pro->el1_stack + PAGE_SIZE, (1 << 0));
